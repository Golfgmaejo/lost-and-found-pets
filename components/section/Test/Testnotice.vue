...
export default {
  ...
  methods: {
    async submit() {
      // ตรวจสอบว่ามีข้อมูลที่ยังเป็น "" หรือ null หรือไม่
      ...
      if (this.$refs.form.validate()) {
        ...
        // Convert lostDate to timestamp using parseISO
        const lostDateTimestamp = this.form.lostDate
          ? adapter.parseISO(this.form.lostDate).getTime()
          : null;
        const data = {
          user_id: "e969bea8-5469-499a-baf8-6c896c556e50",
          name: this.form.name,
          category: "แมว",
          lost_date: lostDateTimestamp,
    ...
    logSelectedDate(selectedDate) {
      if (selectedDate) {
        ...
        this.formattedLostDate = `${day}/${month}/${year}`;

        // Convert the date to timestamp using parseISO
        const timestamp = adapter.parseISO(selectedDate).getTime();

        // Log the formatted date and timestamp
        console.log("Formatted Date:", this.formattedLostDate);
        console.log("Timestamp:", timestamp);

        // Update the form data
        this.form.lostDate = selectedDate; // Keep the ISO format for backend
        this.menuDate = false;
      }
    },
  },
