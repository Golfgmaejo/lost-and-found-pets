<template>
  <v-data-table
    :headers="headers"
    :items="petList"
    :sort-by="[{ key: 'find_date', order: 'asc' }]"
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>หาเจ้าของ</v-toolbar-title>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" md="6">
                    <v-text-field v-model="editedItem.find_date" label="วันที่พบ"></v-text-field>
                  </v-col>
                  <v-col cols="12" md="6">
...
const fetchPets = async () => {
  try {
    const response = await axios.get('http://localhost:5000/api/find_owner/getAll_find_owner');
