<template>
  <v-container>
    ...
    <h5 class="text-portfolio">
      กรุณากรอกข้อมูลให้ครบถ้วนเพื่อให้ผู้ที่พบสัตว์เลี้ยงจะสามารถติดต่อกลับได้
    </h5>
      ...
      <v-row>
        ...
        <v-col cols="12" sm="6">
          ...
          >
            ...
            <v-time-picker
              v-model="form.findowner_time"
              @input="updateFindownerTime"
              scrollable
              format="24hr"
            ></v-time-picker>
          </v-menu>
...
export default {
  ...
  data() {
    return {
      ...
      valid: false,
      autoCloseTime: 3000,
      selectedDate: this.formatDate(this.petData?.findowner_date) || null,
      form: this.petData
        ? { ...this.petData }
        : {
            gender: "",
  ...
  watch: {
    petData(newData) {
      ...
      this.selectedDate = this.formatDate(newData?.findowner_date) || null;
  ...
  computed: {
    ...
    formattedFindownerTime() {
      if (this.form.findowner_time) {
        const [hours, minutes] = this.form.findowner_time.split(":");
  ...
  methods: {
    async handleFileUpload() {
      ...
      try {
        ...
        const response = await axios.put(
          `https://lost-and-found-pets-backend.onrender.com/api/image/update_images_find_owner`,
          formData,
          {
            headers: {
    ...
    async submit() {
      ...
      if (this.valid) {
        ...
        const data = {
          ...this.form,
          findowner_time: this.formattedFindownerTime,
        ...
        axios
          .put(
            `https://lost-and-found-pets-backend.onrender.com/api/find_owner/update_find_owner/${this.petData.id}`,
            data
          )
          .then((response) => {
            toast.success("อัปเดตข้อมูลสำเร็จ", { autoClose: this.autoCloseTime });
            this.$emit("updatefindownerpet");
          })
          .catch((error) => {
            console.error(error);
    ...
    resetForm() {
      ...
      this.center = [this.markerLatLng.lat, this.markerLatLng.lng];
      this.selectedDate = this.formatDate(this.petData?.findowner_date) || null;
      this.form.findowner_time = this.petData?.findowner_time || null;
    ...
    updateFindownerTime(time) {
      this.form.findowner_time = time;
      this.menuTime = false;
    },
    updateFindownerDate() {
      if (this.selectedDate) {
        this.form.findowner_date = this.formattedFindownerDate;
        this.menuDate = false;
      }
    },
    formatDate(dateStr) {
